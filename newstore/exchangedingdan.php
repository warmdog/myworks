...
$sql="select * from new_users_exchange where openid='$openid'  order by id desc";
$row=$pdo->query($sql);
...
<!--页面内容-->
		...
		<?php foreach ($row as $key) {
			$goods=$key['exchange'];
			$sql22="select imgurl ,bonus from new_exchange where goodsname='$goods'";
			$qwe=$pdo->query($sql22);
...
?>
		<div style="border:solid 1px #ddd;padding:10px;height:90px;">
				...
				<div style="width:60%;float:left">
					...
					<b style="font-size:14px;color:#E78900;"><text>积分：</text><text><?php echo $fee;?></text></b><br/>
				</div>
		</div>
		<?php }?>
</body>
